    ...
    <div class="my-6 flex justify-between">
        <div class="flex space-x-6">
            <div class="flex items-center space-x-2" @click="likePost(post.id)">
                ...
                <p class="text-xs text-gray-500">{{ post.likes_count }} likes</p>
            </div>

            <RouterLink :to="{name: 'postview', params: { pk: post.id}}" class="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                <p class="text-xs text-gray-500">{{ post.comments_count }} comments></p>
            </RouterLink>
        </div>
        <div>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
